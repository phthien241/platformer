...
export default class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    if(tiles){
      const layer = map.createLayer("Tide Layer 1", tiles)
      console.log(layer)
    }
    this.createAnims();

    this.player = this.physics.add
      .sprite(0, this.height / 2 - 20, "player")
      .setOrigin(0, 1)
      .setGravityY(300);
    this.player.setCollideWorldBounds(true);
    this.eagle = this.physics.add
      .sprite(this.width / 2, this.height / 2 - 20, "eagle")
      .play("eagle")
      .setOrigin(0, 1)
      .setImmovable(true)
      .setVelocityY(100);
  ...
  handlePlayerEnemyCollision(player: any, enemy: any) {
    ...
    if (player.body.bottom <= enemy.body.top + 10) {
      ...
      const deathAnimation = this.add.sprite(
        this.eagle.x + this.eagle.body.width / 2,
        this.eagle.y - this.eagle.body.height / 2,
        "enemy-deadth"
      );
